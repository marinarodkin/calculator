...
class Calculator extends Component {
  state = {
    ...
    callOperator = () => {
      ...
      if (displayValue > 999999999)  displayValue = "too large";
      ...
      this.setState({ displayValue, selectedOperator, storedValue: updateStoredValue, isNewNumber: true, isAfterResult: true });
    }
  ...
  setOperator = value => {
    ...
    let { displayValue, selectedOperator, storedValue } = this.state;
    if(displayValue === "too large") return;
    ...
    this.setState({ displayValue, selectedOperator, storedValue, isNewNumber: true });
  }

  updateDisplay = value => {
    let { displayValue,  isNewNumber, isAfterResult } = this.state;

    // prevent multiple occurences of '.'
    if (value === '.' && displayValue.includes('.')) value = '';

    if (value === 'ce') {
      //if after clean display
      if (isAfterResult === true) displayValue = '0';
    ...
    this.setState({ displayValue, isNewNumber: false, isAfterResult: false });
  }
